...
func _draw() -> void:
	...
	Paint.text(self,"SINGLE GAME" if GameManager.single_game else "MISSION %d / 7    LOOP %d" % [GameManager.round_index+1,Global.current_loop],Vector2(765,66),23)
	...
	Paint.text(self,"PLAY AGAIN OR PICK ANOTHER GAME AFTER THIS" if GameManager.single_game else "LIVES %d    SCORE %05d    STREAK %d" % [Global.lives,Global.score,Global.streak],Vector2(453,467),19)
