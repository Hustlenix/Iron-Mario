...
(async()=>{
 ...
 page.on('console',m=>{if(/SCRIPT ERROR|Parse Error|Failed to load|ERROR:/.test(m.text()))failures.push(m.text());});
 ...
 phone.on('console',m=>{if(/SCRIPT ERROR|Parse Error|Failed to load|ERROR:/.test(m.text()))failures.push(m.text());});
